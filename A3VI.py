...
print("A3VI收到的AUSF发送的消息明文为：", m_AUSF_A3VI)
...
print('验签成功！！！') if curve.verify(signature_AUSF, b_cipher_h_AUSF, pk_Ope)is True else print('验签失败！！！')
...
data_ST = {'CH_UE': message_AUSF['CH_UE'], 'N': message_AUSF['N'], 'RG_Ope': message_AUSF['RG_Ope'],
           'RG_A3VI': keys, 'ST': ST}
print('上链成功！票据注册成功！')   #  到这一步后我们假装上链成功
...
print("接收到的密钥协商材料为：", m_EC_A3VI)  # {'PID_UE': UE_EC['PID_UE'], 'm_UE': m_UE, 'A_UE': A_UE, 'B_UE': UE_EC['B_UE']}
...
print('A3VI计算出的会话密钥材料K为：', K_A3VI.xy)
...
print('A3VI计算出的会话密钥[Int类型]为：', SK_A3VI)
